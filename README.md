...
编辑 `~/.care-assistant/config.json`：

```json
{
  "base_token": "你的飞书多维表格 Base Token",
  "table_id": "你的记录表 ID",
  "user_open_id": "你的飞书用户 Open ID",
  "folder_token": "你的飞书文件夹 Token",
...
# 创建多维表格获取 base_token 和 table_id
lark-cli base +base-create "CARE 助理"

# 获取 folder_token（在飞书云空间中创建文件夹）
# 记录文件夹 URL 中的 token
```

### 首次使用

在 Claude Code 中随便说一句话即可自动触发 Setup：
...
├── auto-dedup.py                      # 自动去重脚本
├── dedup-and-tasks.py                 # 去重+过期待办处理
├── organize-records.py                # 记录整理脚本
├── extract-article.py                 # 网页内容提取
└── references/
