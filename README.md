...
> 基于 [lark-cli](https://github.com/larksuite/cli) 的个人效率助理。
...
CARE 助理由**1个主 Skill + 3个子 Skill**组成，它们可以独立使用，也可以配合使用。
...
│                    （主路由 Skill）                       │
│                                                          │
│  作用：识别你的意图，自动分发到对应的子 Skill              │
│  触发：你说任何话都会先经过它                              │
└──────────────┬──────────────┬──────────────┬────────────┘
               │              │              │
       ┌───────▼──────┐ ┌────▼─────┐ ┌─────▼──────┐
       │ care-capture │ │ recording│ │ care-review│
       │ 随手记       │ │ 录音转写  │ │ 回顾整理    │
       └──────────────┘ └──────────┘ └────────────┘
```
...
| Skill | 作用 | 何时被调用 |
|-------|------|-----------|
| **care-assistant** | 主路由，识别意图并分发 | 每次对话自动触发 |
| **care-capture** | 记录待办/灵感到飞书表格 | 识别到要记录的内容时 |
| **care-recording** | 处理录音转写，提取信息 | 识别到"录音"、"转写"等词时 |
| **care-review** | 回顾记录，生成分析文档 | 识别到"回顾"、"整理"等词时 |
...
你说："明天下午3点开会"
         ...
         ▼
┌─────────────────┐
│ care-assistant  │ 识别意图：这是一个待办
│   (主路由)      │
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│  care-capture   │ 写入飞书表格 + 创建飞书任务
│   (随手记)      │
└─────────────────┘
         │
         ▼
...
安装命令 `cp -r skills/* ~/.claude/skills/` 会把四个 Skill 复制到 Claude Code 的技能目录：

- 你说任何话，Claude Code 会自动加载 `care-assistant`（主路由）
- 主路由根据你的话，决定是否调用其他子 Skill
- 所有 Skill 共享 `~/.care-assistant/config.json` 配置

### 既可以单独用，也可以配合用

| 使用方式 | 说明 |
|---------|------|
| **配合使用（推荐）** | 安装全部四个 Skill，主路由自动识别意图并调用对应 Skill |
| **单独使用** | 只安装某个子 Skill（如只用 care-capture 记录），手动触发 |
