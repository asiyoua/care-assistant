...
**目标**：确保"原文转写"部分清晰可读，无论原始录音文本是什么格式。

**检测原始格式并处理：**

1. **有时间戳格式**（如 `00:00`、`1:23`）
   ```
   采访者  00:00原文内容...
   受访者  00:04原文内容...
   ```
...
3. **纯段落文本**（既无时间戳也无列表）
   ...
   - 分析段落结构
   - 按段落双换行（段落间空一行）
   - 段落内每 ~80-100 字符换行一次（保持可读性）
   - 避免出现超长段落

**处理流程：**
```bash
# 1. 检测格式
if grep -qE '^[^[:space:]]+[[:space:]]+[0-9]+:[0-9]+' "$转写内容"; then
  格式类型="时间戳"
elif grep -qE '^[[:space:]]*[-*][[:space:]]|[[:space:]]*[0-9]+[.\)]' "$转写内容"; then
  格式类型="列表"
else
  格式类型="段落"
fi
